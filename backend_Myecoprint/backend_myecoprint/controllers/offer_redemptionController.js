...
async function getBarcodeInformation(userId, offerId, storeId) {
  try {
    ...
    if (!barcodeInfo) {
      console.error('Barcode information not found for user ID:', userId, 'offer ID:', offerId, 'store ID:', storeId);
      return null;
    }
...
exports.redeemPoints = async (req, res) => {
  try {
    ...
    const userPoints = await pointController.getTotalPointsByUserId(user.user_id);
    ...
    const barcodeInfo = await getBarcodeInformation( user.user_id, offerId, storeId);
    ...
    try {
      const redemption = await redeemPointsWithOfferDiscount(storeId, offerId, pointsRedeemed, user.user_id);
      ...
      await createTransactionRecord(user.user_id, storeId, offerId, -pointsRedeemed, 'redemption');
      res.status(200).json({
